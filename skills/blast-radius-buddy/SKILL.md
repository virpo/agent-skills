...
9. Give the normalized report, captured diff, and explicit gate state to `scripts/github-review.mjs prepare`. Pass the same report, diff, gates, body, and comments to `submit`; it recomputes them, derives `COMMENT` or `APPROVE`, and rechecks the exact head at the write boundary immediately before POST. Update the marker with the final review link.
